...
const { money, updateDonation, getDonationCurrency } = require('./donations');
...
function buildDonationLogEmbed(record, guildConfig) {
  const symbol = getDonationCurrency(guildConfig.id).currencySymbol;
...
async function postDonationLog(discordGuild, record) {
  ...
  const thread = await forum.threads.create({
    name: `${money(record.amount, getDonationCurrency(discordGuild.id).currencySymbol)} · ${who}`.slice(
      0,
      100
    ),
